...
**Known gap, found by live testing against real credentials (2026-09-14), not yet fixed:** `documentAI.service.ts` calls Document AI's *synchronous* `processDocument` API, which caps out at 30 pages — a real DGA TOR PDF (`TOR-DGA-69-0229.pdf`, 43 pages) failed extraction outright with `Document pages exceed the limit: 30 got 43`. Nothing here falls back to Document AI's asynchronous/batch API (which writes results to GCS and requires polling) for documents over that limit — a TOR whose *only* substantial document is long enough to hit this cap currently fails extraction entirely rather than degrading gracefully, and (per the terminal-state invariant above) sits without a retry path once `EXTRACTION_MAX_ATTEMPTS` is exhausted. Flagged as a known gap, not silently worked around; worth fixing before this pipeline is trusted on agencies whose TORs commonly run long.
...
**Correction (2026-09-14), from the first real credentials this project ever ran against:** `gemini-3.5-flash-lite` does not exist on this project's actual model catalog — every model name tried except one 404'd as *"not found or your project does not have access to it,"* in both `us-central1` and `asia-southeast1`: `gemini-3.5-flash-lite`, `gemini-3-flash`, `gemini-2.5-flash-lite`, `gemini-2.0-flash` (also the *old* default — also gone), `gemini-1.5-flash`. Only **`gemini-2.5-flash`** — a full Flash-tier model, not Lite — worked. The reasoning above for wanting a Lite-tier model stands unchanged; it just isn't available on this project yet. `VERTEX_AI_MODEL` now defaults to `gemini-2.5-flash` (`gcpConfig.ts`), with the discrepancy documented in code rather than silently re-justified after the fact. Revisit once a lighter tier is confirmed reachable — this was a live API-catalog fact, not a design decision, and needs re-checking rather than trusting either value going stale.

Also corrected the same day: `VERTEX_AI_LOCATION` moved from `us-central1` to **`asia-southeast1`** — lower latency for a Bangkok-based platform, and no persisted resource tied to this value (unlike `GCP_LOCATION`/`DOC_AI_PROCESSOR_ID`, which stay on `us` because the Document AI processor was already created there and processors can't be moved between regions after creation — mixed regions across the two Google services is fine, they're independent).
