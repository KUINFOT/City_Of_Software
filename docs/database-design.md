...
Wired up as of that work: `GET/POST/PATCH /api/review/*` (the approval queue, `POST /api/review/tors` for admin-created manual TORs — US-041, and `PATCH /api/review/queue/:id/fields` for field corrections — US-043, both Section 13.11–13.13), `GET /api/tors*` (the first Tor-facing read API), `GET /api/documents/:id/file` (original document bytes, US-043), `GET /api/extraction/ai-jobs*`, `GET /api/extraction/health` (per-source adapter health — US-044, Section 13.12), `GET /api/audit-log` (US-045, Section 13.14), a cron-driven scheduler (off by default), cross-record duplicate scoring, and the `procurement_stats` outlier aggregation (triggered manually or on schedule, not yet automatically nightly-only).
