...
export function getSourcesHealth(): Promise<SourceHealth[]> {
  return request("/extraction/health");
}

export function listExtractionSources(): Promise<ExtractionSource[]> {
  return request("/extraction/sources");
}

export function listScrapeJobs(params?: { sourceId?: string; limit?: number }): Promise<ScrapeJob[]> {
  const query = new URLSearchParams();
  if (params?.sourceId) query.set("sourceId", params.sourceId);
  if (params?.limit) query.set("limit", String(params.limit));
  const qs = query.toString();
  return request(`/extraction/jobs${qs ? `?${qs}` : ""}`);
}

/** Bounded by design — a UI-triggered run should never kick off an unbounded crawl. */
export function triggerSourceRun(sourceId: string): Promise<RunResult> {
  return request(`/extraction/sources/${sourceId}/run`, {
    method: "POST",
